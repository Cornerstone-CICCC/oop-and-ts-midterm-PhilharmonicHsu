...
export class ProductItem extends Component {
  ...
  render() {
    ...
    productItem.innerHTML = `
        ...
        >
        <h2 class="text-xl truncate">${this.props.product.title}</h2>
        <p class="text-normal font-bold text-green-500 mb-3">$${this.props.product.price}</p>
        <button class="add-to-cart bg-amber-500 px-4 py-2 w-full border-1 border-black border-solid rounded-md hover:bg-amber-400">
            Add to Cart
        </button>
      `;
